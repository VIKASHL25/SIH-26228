...
class ProtectedInferenceRecord(BaseModel):
    ...
    inference_config_hash_sha256: Optional[str] = None
...
class ReplayProtectionRegistry:
    ...
    def check_sequence(self, record: ProtectedInferenceRecord) -> Tuple[bool, Optional[str]]:
        """Enforce one contiguous accepted inference stream in this registry."""
        if not self.entries:
            return False, None
        expected = self._last_sequence_number + 1
        if record.sequence_number != expected:
            return True, (
                f"Sequence number {record.sequence_number} is not the next accepted "
                f"sequence number {expected}."
            )
        return False, None
    ...
    def register(self, record: ProtectedInferenceRecord) -> bool:
        ...
        sequence_violation, _ = self.check_sequence(record)
        if is_replay or sequence_violation:
            return False
...
class CryptographicProvenanceEngine:
    ...
    def __init__(
        ...
        configured_secret_file = secret_key_file or os.environ.get("CV_INFERENCE_SECRET_KEY_FILE")
        if secret_key is not None and configured_secret_file:
            raise ValueError("Provide secret_key or secret_key_file, not both.")
        if configured_secret_file:
            with open(configured_secret_file, "rb") as key_file:
                loaded_key = key_file.read().strip()
            if not loaded_key:
                raise ValueError("Configured HMAC secret key file is empty.")
        ...
        # An in-memory registry is always present; callers that need restart
        # persistence can provide ReplayProtectionRegistry(registry_file=...).
        self.replay_registry = replay_registry or ReplayProtectionRegistry()
    ...
    @classmethod
    def canonicalize_inference_config(
        cls,
        config: Dict[str, Any]
    ) -> Tuple[Dict[str, Any], str]:
        """Canonicalize execution/post-processing settings separately from preprocessing."""
        return cls.canonicalize_config(config)

    @classmethod
    def compute_inference_config_hash(
        cls,
        config: Dict[str, Any]
    ) -> str:
        _, canonical_json = cls.canonicalize_inference_config(config)
        return hashlib.sha256(canonical_json.encode('utf-8')).hexdigest()
    ...
    def compute_binding_hash(
        ...
        Payload includes inference configuration when supplied. The legacy
        payload is retained for records created before that field existed.
        """
        ...
        if inference_config_hash is not None:
            canonical_str += f"|{inference_config_hash.lower()}"
    ...
    def create_protected_record(
        ...
        # Inference configuration is intentionally distinct from preprocessing.
        inference_cfg_dict = None
        inference_cfg_hash = None
        if inference_config is not None:
            inference_cfg_dict, _ = self.canonicalize_inference_config(inference_config)
            inference_cfg_hash = self.compute_inference_config_hash(inference_cfg_dict)
        ...
        binding_h = self.compute_binding_hash(
            img_hash, m_hash, cfg_hash, preds_canonical_json, ts, generated_nonce,
            sequence_number, inference_cfg_hash
        )
        ...
        return ProtectedInferenceRecord(
            ...
            inference_config=inference_cfg_dict,
            inference_config_hash_sha256=inference_cfg_hash,
    ...
    def verify_record(
        ...
        expected_inference_config: Optional[Dict[str, Any]] = None,
        ...
        if record.preprocessing_config is not None:
            ...
            if cfg_h.lower() != record.preprocessing_config_hash.lower():
                ...
                field_verifications["preprocessing_config_hash"] = False
            else:
                field_verifications["preprocessing_config"] = True
                field_verifications["preprocessing_config_hash"] = True
        ...
        # 3b. Explicit inference configuration verification
        if record.inference_config is not None:
            inference_cfg_h = self.compute_inference_config_hash(record.inference_config)
            inference_hash_matches = (
                record.inference_config_hash_sha256 is not None and
                inference_cfg_h.lower() == record.inference_config_hash_sha256.lower()
            )
            field_verifications["inference_config"] = inference_hash_matches
            field_verifications["inference_config_hash"] = inference_hash_matches
            if not inference_hash_matches:
                for field in ("inference_config", "inference_config_hash_sha256"):
                    if field not in tampered_fields:
                        tampered_fields.append(field)
        elif record.inference_config_hash_sha256 is not None:
            field_verifications["inference_config"] = False
            field_verifications["inference_config_hash"] = False
            tampered_fields.extend(["inference_config", "inference_config_hash_sha256"])

        if expected_inference_config is not None:
            expected_inference_hash = self.compute_inference_config_hash(expected_inference_config)
            if expected_inference_hash.lower() != (record.inference_config_hash_sha256 or "").lower():
                for field in ("inference_config", "inference_config_hash_sha256"):
                    if field not in tampered_fields:
                        tampered_fields.append(field)
                field_verifications["inference_config"] = False
        ...
        if record.predictions_hash_sha256 is not None:
            ...
            if recomputed_preds_hash.lower() != record.predictions_hash_sha256.lower():
                ...
                tampered_fields.append("predictions_hash_sha256")
                field_verifications["predictions"] = False
                field_verifications["predictions_hash_sha256"] = False
            else:
                field_verifications["predictions"] = True
                field_verifications["predictions_hash_sha256"] = True

        # The ISO representation is metadata, but it must agree with the
        # protected numeric timestamp when present.
        if not math.isfinite(record.timestamp_utc):
            field_verifications["timestamp_utc"] = False
            if "timestamp_utc" not in tampered_fields:
                tampered_fields.append("timestamp_utc")
        if record.timestamp_iso is not None:
            try:
                expected_timestamp_iso = datetime.fromtimestamp(
                    record.timestamp_utc, tz=timezone.utc
                ).isoformat()
                timestamp_matches = (record.timestamp_iso == expected_timestamp_iso)
            except (OverflowError, OSError, ValueError):
                timestamp_matches = False
            field_verifications["timestamp_utc"] = timestamp_matches
            field_verifications["timestamp_iso"] = timestamp_matches
            if not timestamp_matches:
                tampered_fields.extend(
                    field for field in ("timestamp_utc", "timestamp_iso")
                    if field not in tampered_fields
                )
        ...
        if original_record is not None:
            if record.record_id != original_record.record_id:
                tampered_fields.append("record_id")
            ...
            if record.inference_config != original_record.inference_config:
                if "inference_config" not in tampered_fields:
                    tampered_fields.append("inference_config")
            if record.inference_config_hash_sha256 != original_record.inference_config_hash_sha256:
                if "inference_config_hash_sha256" not in tampered_fields:
                    tampered_fields.append("inference_config_hash_sha256")

            if record.predictions_hash_sha256 != original_record.predictions_hash_sha256:
                if "predictions_hash_sha256" not in tampered_fields:
                    tampered_fields.append("predictions_hash_sha256")
        ...
        expected_binding = self.compute_binding_hash(
            ...
            record.inference_config_hash_sha256
        )
        ...
        if registry is not None and check_replay:
            ...
            sequence_violation, sequence_reason = registry.check_sequence(record)
            if sequence_violation:
                field_verifications["sequence_number"] = False
                if "sequence_number" not in tampered_fields:
                    tampered_fields.append("sequence_number")
            else:
                field_verifications.setdefault("sequence_number", True)
        else:
        ...
        if verification_passed:
            ...
            if sequence_violation:
                tamper_reasons.append("SEQUENCE VIOLATION (record is not the next accepted sequence number)")
            ...
            if any(f in tampered_fields for f in ["image_hash_sha256", "model_digest_sha256", "preprocessing_config", "predictions", "inference_config", "inference_config_hash_sha256"]):
