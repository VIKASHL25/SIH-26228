...
class ModelFingerprinter:
    ...
    def fingerprint_dummy_or_callable(
        ...
        """
        Compatibility method for governance engine. If model_or_fn is a string path to a model,
        delegates to fingerprint_model. Otherwise computes behavioral baseline.
        """
        ...
        samples = samples[:num_eval] if samples else []
        if not samples:
            return BehavioralFingerprintResult(
        ...
        confidences = []
        predictions = []
        num_classes = len(dataset.categories) if hasattr(dataset, "categories") and dataset.categories else 10
        cat_names = [dataset.categories.get(i, f"class_{i}") for i in range(num_classes)] if hasattr(dataset, "categories") else [f"class_{i}" for i in range(num_classes)]

        for idx, s in enumerate(samples):
            if callable(model_or_fn):
                try:
                    img_p = s.image_path if hasattr(s, "image_path") else str(s)
                    res = model_or_fn(img_p)
        ...
        anomalous = False
        summary_lines = []
        if entropy < 0.5 and num_classes > 2:
            anomalous = True
